...
CLASS zcl_219000_demo_0103 IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    result += 1. out->write( result )."Zuweisungsoperator für die Addition
    result -= 1. out->write( result )."Zuweisungsoperator für die Subtraktion
    result *= 1. out->write( result ). "Zuweisungsoperator für die Multiplikation
    result /= 1. out->write( result )."Zuweisungsoperator für die Division

    ADD -1 TO result. out->write( result )."Inkrementieren/Dekrementieren

    "Numerische Funktionen
    result = abs( -3 ). out->write( result ). "Betrag
    result = ipow( base = 5 exp = 3 ). out->write( result ). "Potenz
  ENDMETHOD.
